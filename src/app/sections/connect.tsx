...
const ConnectWithMe: React.FC = () => {
  ...
  const renderSocialLinks = () => {
    return socials.map((link, index) => {
      ...
      return (
        ...
        >
          <div
            className="block h-full bg-card/40 backdrop-blur-sm rounded-2xl border border-border hover:border-opacity-0 transition-all duration-300 overflow-hidden group relative"
            style={{
          ...
          >
            ...
            <div className="relative flex flex-col items-center text-center p-6 h-full z-10">
              ...
              <span className="text-l font-medium text-foreground mb-1">{link.name}</span>
            </div>
          </div>
        </motion.div>
      );
  ...
  return (
    <section id="connect" className="py-24 relative bg-background">
      ...
      <div className="absolute -left-20 top-40 w-72 h-72 bg-accent/5 rounded-full blur-3xl" />
      <div className="absolute -right-20 bottom-40 w-80 h-80 bg-primary/5 rounded-full blur-3xl" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div
          ...
          viewport={{ once: true, margin: '-100px' }}
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">{renderSocialLinks()}</div>
        </motion.div>
